...
|------------|---------------|
| Maximum performance (no NAT, no veth) | No network isolation (security concern) |
| No IP address management needed | Port conflicts between containers |
| Direct access to host network | Can't run multiple containers with same port |
| | Less portable |
...
| Advantages | Disadvantages |
|------------|---------------|
| Containers on different hosts communicate seamlessly | More complex |
| Scalable (add more hosts easily) | Slightly slower (encapsulation overhead) |
| Isolated overlay networks | Requires clustering/orchestration |
| Built into Docker Swarm | Higher CPU usage |
...
**Use Case**:
- API Gateway needs access to both frontend (public) and backend (internal) networks.
- Monitoring container needs access to all networks.

---

## 12. Common Networking Commands
...
```bash
# Check if container is on user-defined network
docker network inspect my-network
...
docker run --network backend my-app
# No -p flag, only accessible via other containers
```

### 5. Use Network Policies (Kubernetes)
```bash
...
```bash
# Default MTU is 1500
# For overlay networks, reduce MTU to avoid fragmentation
docker network create \
...
```
Tier 1 (Web): Public network, port 80/443
Tier 2 (App): Internal network, no public ports
Tier 3 (DB): Private network, no external access
```
...
│                                                         │
│  Host network offers best performance but less security│
│                                                         │
│  Overlay networks enable multi-host communication      │
│                                                         │
│  Macvlan gives containers physical network identity    │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
| Concept | Details |
|---------|---------|
| **Bridge** | Default driver, single host, NAT enabled |
| **Host** | No isolation, best performance, port conflicts |
| **None** | No networking, airgapped containers |
| **Overlay** | Multi-host, VXLAN tunneling, requires Swarm |
| **Macvlan** | Physical network identity, no NAT |
| **veth** | Virtual Ethernet pair connecting container to host |
| **NAT** | Network Address Translation, hides container IPs |
| **DNS** | Docker internal DNS (127.0.0.11) for container names |
| **Port Mapping** | `-p host:container` exposes ports to host |
| **Network Isolation** | Containers on different networks can't communicate |

### Network Driver Selection Guide

| Need | Recommended Driver |
|------|-------------------|
| Single host, simple apps | Bridge |
| High performance, single host | Host |
| Multi-host, Swarm | Overlay |
| Physical network access | Macvlan |
| Maximum security | None + manual config |
| Production microservices | User-defined Bridge |
...
1. **What is the difference between Bridge and Host network modes?**
2. **How does Docker provide DNS resolution for container names?**
3. **Why should you use user-defined networks instead of the default bridge?**
4. **What is the purpose of a veth pair in Docker networking?**
5. **How do you expose a container port to the host?**
6. **What is the difference between Overlay and Bridge networks?**
7. **How can you troubleshoot if two containers can't communicate?**
8. **What are the security implications of using the Host network mode?**
9. **How does Macvlan differ from Bridge networking?**
10. **What command would you use to connect a running container to a new network?**
