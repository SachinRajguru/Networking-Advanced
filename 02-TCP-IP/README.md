...
**TCP/IP** (Transmission Control Protocol/Internet Protocol) is the fundamental communication protocol suite that powers the internet and most modern networks. It is not a single protocol but a collection of protocols working together at different layers.

### Why TCP/IP Exists

| Problem | Solution |
|---------|----------|
| Different computer systems and networks couldn't communicate because they used incompatible protocols | TCP/IP provides a standardized set of rules that all devices follow |
...
| Step | Action | Details |
|------|--------|---------|
| **Step 1 - SYN** | Synchronize | Client sends SYN packet with initial sequence number (ISN). ISN is randomly chosen for security. SYN flag is set in TCP header. No data is sent yet. |
| **Step 2 - SYN-ACK** | Synchronize-Acknowledge | Server responds with its own SYN (with its ISN). Also ACKs client's SYN (client's ISN + 1). Both SYN and ACK flags are set. Server allocates resources for connection. |
| **Step 3 - ACK** | Acknowledge | Client acknowledges server's SYN (server's ISN + 1). ACK flag is set. Client can start sending data in this packet. Connection is now ESTABLISHED. |

### Why Three Steps?

| Reason | Explanation |
|--------|-------------|
| **Two-way wouldn't work** | Both sides need to agree and acknowledge. Prevents half-open connections. Ensures both sides have allocated resources. |
| **Four-way would be wasteful** | SYN-ACK combines server's SYN and ACK into one packet. More efficient. |
...
| 3000 | Many dev servers | TCP | Node.js, etc. |
| 8080 | Alternative HTTP | TCP | Alternative web server |
...
## 6. TCP/IP Model (4 Layers - More Practical)
...
## 8. Understanding Network Traffic Flow
...
| **Transport Layer** | TCP adds port numbers (443) |
| **Network Layer** | IP adds source and destination IP |
| **Data Link Layer** | Adds MAC addresses for next hop |
| **Physical Layer** | Converts to electrical signals |

---

## 9. Key Concepts for DevOps
...
│                                                         │
│  Most DevOps work happens at Layer 4 (Transport)       │
│  and Layer 7 (Application)                             │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
| **Ephemeral Ports** | 32768-60999 (Linux), 49152-65535 (Windows) |
...
2. **How many steps are in a TCP handshake?**
3. **What port does HTTPS use?**
4. **What is the ephemeral port range on Linux?**
5. **Which OSI layer handles routing?**
6. **What is a socket?**
7. **Why does TCP use a 3-way handshake instead of 2-way?**
8. **What is the purpose of the TIME_WAIT state?**
