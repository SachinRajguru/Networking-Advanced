...
A **CDN** (Content Delivery Network) is a geographically distributed network of servers that delivers content to users from the nearest location.
...
| **TTL-Based** | Remove based on expiration time |
| **Adaptive Algorithm** | Combines popularity, recency, size, TTL, origin load |
...
|--------------|---------------|--------|
| **Static Assets** (CSS, JS, Images with hash) | `public, max-age=31536000, immutable` | Content hash in filename, never changes |
| **Images** (without hash) | `public, max-age=86400` | May update, revalidate daily |
| **HTML** | `public, max-age=300, must-revalidate` | Changes frequently, revalidate after 5 minutes |
| **API Responses** | `private, no-cache` | User-specific, don't cache at CDN |
| **Sensitive Data** | `private, no-store` | Never cache anywhere |
...
|----------|-------------|
| **Increase TTL** | Cache content longer |
| **Query String Consistency** | Ensure consistent parameters |
| **Normalize Headers** | Don't vary cache on unnecessary headers |
| **Prefetch Popular Content** | Push popular content to edge before requests |
...
| Method | Description |
|--------|-------------|
| **Cache Purging/Invalidation** | API call to remove specific URLs |
| **Versioning** | Use versioned URLs (e.g., style.v2.css) |
| **Cache-Control Headers** | Short TTL for frequently changing content |
...
## 13. Monitoring CDN Performance

### Key Metrics:

| Metric | Description | Goal |
|--------|-------------|------|
| **Cache Hit Ratio** | Cache Hits / Total Requests × 100 | > 90% |
| **Bandwidth Savings** | CDN bandwidth used vs Origin bandwidth saved | Maximize |
| **Response Time** | Time to first byte (TTFB) | < 100ms for cached content |
...
│                                                         │
│  HTTP/2 and HTTP/3 improve CDN performance             │
│                                                         │
│  Brotli compression reduces file sizes significantly   │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
| **Origin Shield** | Intermediate cache layer protecting origin |
| **Anycast** | Routing same IP to nearest server |
| **DNS-Based Routing** | DNS returns nearest PoP IP |
| **Cache Invalidation** | Removing old cached content |
| **Versioning** | Using unique URLs to force cache refresh |
| **DDoS Protection** | CDN absorbs attack traffic |
| **WAF** | Web Application Firewall at edge |
| **SSL Termination** | CDN handles encryption, not origin |
| **Brotli** | Compression algorithm (better than gzip) |
| **HTTP/2** | Multiplexing, header compression |
| **HTTP/3** | QUIC protocol, 0-RTT handshake |
...
| Enable HTTP/2 | Yes |
| Enable SSL | Yes (Free or Custom) |
| Set Cache Rules | `/static/*` cache, `/api/*` bypass |
| Monitor Hit Ratio | > 90% target |
| Purge Cache | API call or versioning |
| Use Separate Domain | `cdn.example.com` |
...
3. **Why is TTL important in CDN caching?**
4. **What is the difference between DNS-based routing and Anycast routing?**
5. **How does a CDN reduce latency?**
6. **What is the recommended cache strategy for static assets vs. HTML?**
7. **Why is versioning URLs better than cache purging?**
8. **What is a cache hit ratio and why should you monitor it?**
9. **How does a CDN provide DDoS protection?**
10. **What are the benefits of using a separate domain for CDN content?**
